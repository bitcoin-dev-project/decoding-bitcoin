...
2. **OP_PUSHDATA1, OP_PUSHDATA2, OP_PUSHDATA4**: Push a specific number of bytes onto the stack. The number following the opcode specifies how many bytes of the subsequent data should be pushed.

<OpcodeDataVisualizer></OpcodeDataVisualizer>

3. **OP_1NEGATE**: Pushes the number -1 onto the stack.

4. **OP*PUSHBYTES*[1-75]**: Push 1 to 75 bytes onto the stack. The number in the opcode name indicates how many bytes to push.
