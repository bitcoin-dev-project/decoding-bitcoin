...
### Pay-to-Public-Key (P2PK) Script Decoder Exercise

> [!WARNING] > **Note:** THIS EXERCISE IS NOT COMPLETE. (IGNORE THIS)

In this exercise, you'll implement a function to decode a P2PK ScriptPubKey and extract the public key.
...
function decodeP2PKScript(hexScript) {
// TODO: Implement the decoding logic
// 1. Convert the hex script to bytes
// 2. Extract the public key (remember the first byte indicates the length of the key)
// 3. Return the extracted public key in hex format
}
...
const handleDecode = () => {
const result = decodeP2PKScript(inputScript);
setDecodedKey(result);
};

return (
<div className="container">
<h1>P2PK Script Decoder</h1>
<div>
<input
type="text"
value={inputScript}
onChange={(e) => setInputScript(e.target.value)}
placeholder="Enter P2PK Script"
/>
<button onClick={handleDecode}>Decode</button>
</div>
<div>
<h2>Decoded Public Key:</h2>
<p>{decodedKey || 'No key decoded yet'}</p>
</div>
</div>
);
}
...
1. Implement the `decodeP2PKScript` function in the code editor above.
2. The function should take a hex-encoded P2PK ScriptPubKey as input.
3. Convert the hex script to bytes.
4. Extract the public key, remembering that the first byte indicates the length of the key.
5. Return the extracted public key in hex format.
6. Test your implementation using the input field and "Decode" button in the UI.

### Test Scripts

You can use these P2PK scripts to test your implementation:

1. `4104ae1a62fe09c5f51b13905f07f06b99a2f7159b2225f374cd378d71302fa28414e7aab37397f554a7df5f142c21c1b7303b8a0626f1baded5c72a704f7e6cd84cac`
2. `410411db93e1dcdb8a016b49840f8c53bc1eb68a382e97b1482ecad7b148a6909a5cb2e0eaddfb84ccf9744464f82e160bfa9b8b64f9d4c03f999b8643f656b412a3ac`

#### Hints

-   Use `parseInt(hexString, 16)` to convert hex to decimal.
-   Remember that each byte in hex is represented by two characters.
-   The OP_CHECKSIG at the end of the script is represented by 'ac' in hex.
...
def decode_p2pk_script(hex_script): # Step 1: Decode the hex script
script_bytes = bytes.fromhex(hex_script)

    # Step 2: Extract the public key (the script is P2PK, so the first byte indicates the length of the key)
    pub_key_length = script_bytes[0]
    pub_key = script_bytes[1:1 + pub_key_length]

    # Step 3: Convert public key to Bitcoin address
    ...
    # Step 6: Take the first 4 bytes of the second SHA-256 hash as checksum
    checksum = sha256_3[:4]
    ...
    num = int.from_bytes(bin_addr, 'big')
    base58_addr = ''
    while num > 0:
        num, rem = divmod(num, 58)
        base58_addr = alphabet[rem] + base58_addr

    # Handle leading zero bytes
    num_leading_zeros = len(bin_addr) - len(bin_addr.lstrip(b'\0'))
    return '1' * num_leading_zeros + base58_addr

# Example usage

hex_script = '4104678afdb0fe5548271967f1a67130b7105cd6a828e03909a67962e0ea1f61deb649f6bc3f4cef38c4f35504e51ec112de5c384df7ba0b8d578a4c702b6bf11d5fac'
address = decode_p2pk_script(hex_script)
