...
export function OP_CHECKSIG(stack, getMessageHash) {
    ...
    // 1. Check if there are at least two items on the stack
    // 2. Pop the public key and signature from the stack
    // 3. Get the message hash from the user
    // 4. Verify the signature
    // 5. Push the result (true/false) onto the stack
}
